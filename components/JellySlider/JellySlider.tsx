...
import { NumberProvider } from './numbers';

export default function JellySlider() {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const [qualityScale, setQualityScale] = useState(0.5);
    const [lightDir, setLightDir] = useState(0);
    const [jellyColor, setJellyColor] = useState([1.0, 0.45, 0.075]);
        ...
        const init = async () => {
            try {
                root = await tgpu.init({
                    device: {
                        optionalFeatures: ['timestamp-query' as GPUFeatureName],
                    },
                });
            } catch (err) {
                console.error('Failed to init TypeGPU', err);
                return;
            }
            ...
            const handleResize = () => {
                if (!canvas) return;
                const newWidth = canvas.width * qualityScale;
                const newHeight = canvas.height * qualityScale;
                ...
                const newRenderBindGroups = [];
                for (let frame = 0; frame < 2; frame++) {
                    newRenderBindGroups.push(
                        root.createBindGroup(sampleLayout, {
                            currentTexture: taaResolver.getResolvedTexture(frame),
                        })
                    );
            ...
            // Expose update functions for controls
            (window as any).updateLightDir = (v: number) => {
            ...
            (window as any).updateJellyColor = (c: number[]) => {
                jellyColorUniform.write(d.vec4f(c[0], c[1], c[2], 1.0));
            };

            (window as any).updateBlur = (v: boolean) => {
                blurEnabledUniform.write(d.u32(v ? 1 : 0));
            };
        };
    ...
    }, [qualityScale]); // Re-run if quality scale changes (though we might want to handle this differently to avoid full re-init)

    // Effects for controls
    useEffect(() => {
        if ((window as any).updateLightDir) {
            (window as any).updateLightDir(lightDir);
        }
    }, [lightDir]);

    useEffect(() => {
        if ((window as any).updateJellyColor) {
            (window as any).updateJellyColor(jellyColor);
        }
    }, [jellyColor]);

    useEffect(() => {
        if ((window as any).updateBlur) {
            (window as any).updateBlur(blurEnabled);
        }
    }, [blurEnabled]);
    ...
    return (
        <div className="relative w-full h-screen bg-gray-900">
            ...
            <div className="absolute top-4 right-4 bg-black/50 backdrop-blur-md p-4 rounded-lg text-white space-y-4 w-64">
                <h2 className="text-xl font-bold mb-2">Controls</h2>

                <div>
                    <label className="block text-sm mb-1">Quality Scale: {qualityScale}</label>
                    <input
                        type="range"
                        min="0.1"
                        max="1.0"
                ...
                <div>
                    <label className="block text-sm mb-1">Light Direction</label>
                    <input
                        type="range"
                        min="0"
                        max="1"
                        step="0.01"
                        value={lightDir}
                        onChange={(e) => setLightDir(parseFloat(e.target.value))}
                        className="w-full"
                    />
                </div>

                <div>
                    <label className="block text-sm mb-1">Jelly Color</label>
                    <div className="flex gap-2">
                        <div
                            className="w-8 h-8 rounded-full cursor-pointer border-2 border-white"
                            style={{ backgroundColor: 'rgb(255, 115, 19)' }}
                            onClick={() => setJellyColor([1.0, 0.45, 0.075])}
                        />
                        <div
                            ...
                            onClick={() => setJellyColor([0.0, 1.0, 0.0])}
                        />
                        <div
                            className="w-8 h-8 rounded-full cursor-pointer border-2 border-transparent hover:border-white"
                            style={{ backgroundColor: 'rgb(0, 100, 255)' }}
                            onClick={() => setJellyColor([0.0, 0.4, 1.0])}
                        />
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor="blur" className="text-sm">Enable Blur</label>
                </div>
            </div>
        </div>
    );
